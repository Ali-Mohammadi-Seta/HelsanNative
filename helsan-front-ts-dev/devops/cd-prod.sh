...
if [ -z "$ZONE35_GITEA_USERNAME" ]; then
  echo "Error: ZONE35_GITEA_USERNAME is not set"
  exit 1
fi
if [ -z "$ZONE35_GITEA_TOKEN" ]; then
  echo "Error: ZONE35_GITEA_TOKEN is not set"
  exit 1
fi
if [ -z "$ZONE35_GITEA_URL" ]; then
  echo "Error: ZONE35_GITEA_URL is not set"
  exit 1
fi
if [ -z "$ZONE35_GITEA_STACK" ]; then
  echo "Error: ZONE35_GITEA_STACK is not set"
  exit 1
fi

CENTRAL_REPO_CLONE_URL="https://${ZONE35_GITEA_USERNAME}:${ZONE35_GITEA_TOKEN}@${ZONE35_GITEA_URL}/${ZONE35_GITEA_STACK}/${STACK}.git"
TEMP_DIR="/tmp/${STACK}"
